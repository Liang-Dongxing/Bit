...
public class CaptchaConfig {
    ...
    public DefaultKaptcha getKaptchaBean() {
        ...
        // 验证码图片宽度 默认为200
        properties.setProperty(KAPTCHA_IMAGE_WIDTH, String.valueOf(BitConfig.getCaptchaWidth()));
        // 验证码图片高度 默认为50
        properties.setProperty(KAPTCHA_IMAGE_HEIGHT, String.valueOf(BitConfig.getCaptchaHeight()));
        // 验证码文本字符大小 默认为40
    ...
    public DefaultKaptcha getKaptchaBeanMath() {
        ...
        // 验证码文本生成器
        properties.setProperty(KAPTCHA_TEXTPRODUCER_IMPL, "com.bit.framework.config.KaptchaTextCreator");
        // 验证码文本字符间距 默认为2
        properties.setProperty(KAPTCHA_TEXTPRODUCER_CHAR_SPACE, "6");
        // 验证码文本字符长度 默认为5
    ...
    private ICaptcha getHutoolCaptcha(CodeGenerator codeGenerator) {
        ...
        if ("png".equals(captchaImageFormat)) {
            LineCaptcha lineCaptcha = new LineCaptcha(BitConfig.getCaptchaWidth(), BitConfig.getCaptchaHeight());
            lineCaptcha.setGenerator(codeGenerator);
            return lineCaptcha;
        } else if ("gif".equals(captchaImageFormat)) {
            GifCaptcha gifCaptcha = new GifCaptcha(BitConfig.getCaptchaWidth(), BitConfig.getCaptchaHeight());
            gifCaptcha.setGenerator(codeGenerator);
            return gifCaptcha;
        }
        return CaptchaUtil.createGifCaptcha(BitConfig.getCaptchaWidth(), BitConfig.getCaptchaHeight());
    }
}
